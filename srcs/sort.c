...
/*   Updated: 2016/12/14 00:07:28 by vdarmaya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_ascii_rev(t_lstent **lst, t_lstent *add, char point)
{
	t_lstent	*prev;
	t_lstent	*tmp;
...
t_lstent	*sort_ascii_rev(DIR *dir, char point)
{
	t_lstent	*lst;
	t_lstent	*tmp;
	t_dirent	*entity;
...
t_lstent	*sort_opt(DIR *dir, t_opt *opt, char *path)
{
	if (!opt)
		...
		return (opt->set_a ? sort_time(dir, 1, path) : sort_time(dir, 0, path));
	else if (opt->set_a)
